...
FilterA <- function(Coverage_Threshold) {
  ...
  TICKERS_PASS_A <- COVERAGE_TABLE$ticker[COVERAGE_TABLE$coverage >= Coverage_Threshold]
  TICKERS_DROP_A <- COVERAGE_TABLE$ticker[COVERAGE_TABLE$coverage < Coverage_Threshold]
...
FilterB <- function(Minutes_Big_Gap, Maximum_N_Big_Gaps, Max_Gap_Allowed) {
  ...
  gap_rows <- lapply(seq_along(ticker_cols_A), function(j) {
    ...
    max_any <- suppressWarnings(max(max_gaps[is.finite(max_gaps)], na.rm = TRUE))
