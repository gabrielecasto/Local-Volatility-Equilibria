...
GetIntradayRange <- function(ticker, from_date, to_date, multiplier = 1,
  ...
  if (httr::status_code(res) != 200) {
    message("Error ", httr::status_code(res), " for ", ticker, " in range ", from, " - ", to)
  ...
  if (!("results" %in% names(js)) || !is.data.frame(js$results) || nrow(js$results) == 0) {
    return(tibble::tibble())
  }
  
  df <- tibble::tibble(
    datetime = as.POSIXct(js$results$t / 1000, origin = "1970-01-01", tz = "UTC") %>%
...
BuildWideIntradayDf <- function(tickers, from_date, to_date, multiplier = 1, timespan = "minute",
                                sleep_sec = 0, api_key = POLYGON_KEY, verbose = FALSE,
                                Patch_Bad_Days = TRUE,
  ...
  # Trading days only
  dates <- tidyquant::tq_get("^GSPC", from = from_date, to = to_date, get = "stock.prices") %>%
    dplyr::distinct(date) %>%
    dplyr::pull(date)
  ...
  for (m in quarterx) {
    ...
    if (Patch_Bad_Days) {
      ...
      for (d0 in unique_days) {
        ...
        na_share <- colMeans(is.na(as.data.frame(df_wide_month[rows_d, ..ticker_cols])))
